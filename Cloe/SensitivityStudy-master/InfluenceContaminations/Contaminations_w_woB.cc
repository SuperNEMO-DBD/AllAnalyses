...
void Contaminations_w_woB(){

  TTree *SeWithB = new TTree("t_SeWithB","Se with B") ;
  TTree *NdWithB = new TTree("t_NdWithB","Nd with B") ;

  NdWithB->ReadFile("results_Se_without_B.txt","NdBInf/F:NdBSup/F:NdBT12/F") ;
  ...
  hSe->GetYaxis()->SetRangeUser(3.4e24,6.2e24) ;

  auto legend = new TLegend(0.547,0.887,0.999,0.981);
  // auto legend = new TLegend(0.558,0.816,0.897,0.997);
  // legend->AddEntry(hNd,"^{82}Se sources, without magnetic field","lep");
  legend->AddEntry(hSe,"^{82}Se sources, with magnetic field 25 G","lep");
  legend->Draw();
  ...
  config_histo1D(hSeBSup,"P","","Energy (MeV)",0,1,kCyan-2) ; hSeBSup->SetMarkerColor(0) ;
  hSeBSup->GetYaxis()->SetRangeUser(2.6,3.4) ;
  hSeBSup->GetXaxis()->SetLabelSize(0.17);
  hSeBSup->GetXaxis()->SetLabelOffset(0.03);
  hSeBSup->GetYaxis()->SetLabelSize(0.1);
  hSeBSup->GetYaxis()->SetTitleSize(0.12);
  hSeBSup->GetYaxis()->SetTitleOffset(0.42);
  hSeBSup->GetYaxis()->SetNdivisions(010);
  ...
  // line3->SetLineColorAlpha(kOrange+2,1) ;
  // line4->SetLineColor(kOrange+2) ;
  // line4->SetLineWidth(6) ;
  // line4->SetLineColorAlpha(kOrange+2,1) ;
  // line5->SetLineColor(kOrange+2) ;
  // line5->SetLineWidth(6) ;
  // line5->SetLineColorAlpha(kOrange+2,1) ;
  // line6->SetLineColor(kOrange+2) ;
  // line6->SetLineWidth(6) ;
  // line6->SetLineColorAlpha(kOrange+2,1) ;
  // line3->Draw("same") ;
  // line4->Draw("same") ;
  // line5->Draw("same") ;
  // line6->Draw("same") ;

  TLine* line7 = new TLine(0,2.7,2,2.7) ;
  line7->SetLineColor(1) ;
  line7->SetLineWidth(2) ;
  line7->Draw() ;

  c->SaveAs("plots/cont_level_B.pdf") ;
