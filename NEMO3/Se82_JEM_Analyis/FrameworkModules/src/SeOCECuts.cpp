...
// Module declaration
#include "JEM_Analysis/SeOCECuts.h"
...
//------------------------------------------------------------
SeOCECuts::SeOCECuts(const std::string theName,
...
// Operations
//-------------------------------------------------------------------------
AppResult SeOCECuts::beginJob(AppEvent* aJob) {
  ...
  nTupleManager->addBranch(name(), "b_entryPoint", &entryPoint_, "entryPoint/I");
...
AppResult SeOCECuts::beginRun(AppEvent* aRun) {
  return AppResult::OK;
}

AppResult SeOCECuts::event(AppEvent* anEvent) {
  ...
  if (!runRecord.is_valid()) {
    std::cout << "-- SeOCECuts - Error Accessing the RunHeader" << std::endl;
    anEvent->listObjects();
  ...
  if (ElectronColl::find(anEvent, theElectronColl)) {        
    ...
    std::cout << "-- SeOCECuts - Electrons not found" << std::endl;
    return AppResult::OK;
  }
  ...
  if (AlphaColl::find(anEvent, theAlphaColl)) {        
    ...
    std::cout << "-- SeOCECuts - Alphas not found" << std::endl;
    return AppResult::OK;
  }
  ...
  if (GammaClusterColl::find(anEvent, theGammaClusterColl)) {        
    ...
    std::cout << "-- SeOCECuts - Gamma Clusters not found" << std::endl;
    return AppResult::OK;
  }
  ...
  if (N3TrackSegmentColl::find(anEvent, theTrackColl)) {        
    ...
    std::cout << "-- SeOCECuts - Tracks not found" << std::endl;
    return AppResult::OK;        
  }
  ...
  if (CalibCaloHitColl::find(anEvent, theCaloHitColl)) {
    ...
    std::cout << "-- SeOCECuts - Calibrated calorimeter hits not found" << std::endl;
    return AppResult::OK;
  }
  ...
  if (N3CalorimeterHitColl::find(anEvent, theRawCaloHitColl)) {
    ...
    std::cout  << "-- SeOCECuts - Raw Calorimeter hits not found"  << std::endl;
    return AppResult::OK;
  }
  ...
  if (N3GeigerHitColl::find(anEvent, theRawGgHitColl)){
    ...
    std::cout << "-- SeOCECuts - Raw Geiger hits not found" << std::endl;
    return AppResult::OK;
  }
  ...
  if (TrueVertexColl::find(anEvent, theTrueVertexColl)){
    ...
    std::cout << "-- SeOCECuts - True Vertex info not found" << std::endl;
    return AppResult::OK;
  }
  ...
  if (!nemorHeader_ci.is_valid()) {
    std::cout << "-- SeOCECuts - Error Accessing the NemorHeader" << std::endl;
    anEvent->listObjects();
    return AppResult::ERROR;
  }
  ...
  // Cut unassociated tracks
  if(trackList.at(0).indexOfScintillator() < 0 or trackList.at(1).indexOfScintillator() < 0 or
     rawCaloHitList.at(trackList.at(0).indexOfScintillator()).isNoisy() or
  ...
  // Cut events where the electrons aren't both reconstructed from the foil
  if(!electrons[0].intersectTheFoil() or !electrons[1].intersectTheFoil()){return AppResult::OK;}
  TwoIntersectFoil_++;

  // Define vertex as midpoint of two intersections
  CLHEP::Hep3Vector vertex = (electrons[0].vertex() + electrons[1].vertex()) / 2.0;
  ...
  // Cut events with same sign tracks
  if(trackSign_[0]*trackSign_[1] > 0){return AppResult::OK;}
  NegativeTracks_++;
  ...
  // Take largest of two probabilities as the external probability and store entry point for electron
  double chiSqExt;
  if(probExt0 > probExt1){
    probExt_ = probExt0;
    if(rawCaloHitList.at(rawCaloHitIds[0]).iobtFlag() == 0)      {entryPoint_ = 0;}
    else if(rawCaloHitList.at(rawCaloHitIds[0]).iobtFlag() == 1) {entryPoint_ = 1;}
    else                                                         {entryPoint_ = 2;}
  } else {
    probExt_ = probExt1;
    if(rawCaloHitList.at(rawCaloHitIds[1]).iobtFlag() == 0)      {entryPoint_ = 0;}
    else if(rawCaloHitList.at(rawCaloHitIds[1]).iobtFlag() == 1) {entryPoint_ = 1;}
    else                                                         {entryPoint_ = 2;}
  }

  // Cut events with internal probability greater than 0.04
  if(probInt_ > 0.04){return AppResult::OK;}
  IntProb4Percent_++;

  // Cut events with external probability less than 0.01
...
AppResult SeOCECuts::endRun(AppEvent* aRun) {
  return AppResult::OK;
}

AppResult SeOCECuts::endJob(AppEvent* aJob) {
  ...
  if (outfile.is_open()) {
  outfile << "======================================================================================" << std::endl;
  outfile << "                     SeOCECuts End of Job Run Time information                     " << std::endl;
  outfile << "======================================================================================" << std::endl;
  outfile <<                                                                                             std::endl;
  ...
  outfile <<                                                                                             std::endl;
  outfile << "======================================================================================" << std::endl;
  outfile << "                      SeOCECuts End of Job Event Cuts Summary                      " << std::endl;
  outfile << "======================================================================================" << std::endl;
  outfile <<                                                                                             std::endl;
  ...
  outfile << std::setw(60) << std::left  << "Only one missed Gg layer before scintillator"
	  ...
	  <<                                                                                             std::endl;
  outfile << std::setw(60) << std::left  << "Tracks have different sign"
  ...
  outfile << std::endl;
  outfile << "======================================================================================" << std::endl;
  outfile << "                     SeOCECuts End of Job Radon information                          " << std::endl;
  outfile << "======================================================================================" << std::endl;
  outfile <<                                                                                             std::endl;
  ...
  outfile << "====================================================================================" << std::endl;
    ...
    std::cout << "SeOCECuts::endJob : could not write the report" << std::endl;
    return  AppResult::ERROR;
  }
