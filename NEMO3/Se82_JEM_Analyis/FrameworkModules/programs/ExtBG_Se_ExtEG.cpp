/*
 File:      ExtBG_Se_ExtEG.cpp
 Author:    James Mott
 
 Description: Measurement of internal backgrounds in the Se_Old foils in the 1e1g channel.
 */
...
: AppBuild(theFramework) {
    ...
    GammaFinder* gFinder = new GammaFinder();
    add(gFinder);

    aModule = new CaloClusterFinder();     add(aModule);
    aModule = new SeExtEGCuts();           add(aModule);
