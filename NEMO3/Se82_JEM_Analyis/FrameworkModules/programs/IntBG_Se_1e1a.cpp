/*
 File:      IntBG_Se_1e1a.cpp
 Author:    James Mott
 
 Description: Measurement of internal backgrounds in the Se foils in the 1e1a channel.
 */
...
: AppBuild(theFramework) {
    ...
    AlphaFinder* alphaModule = new AlphaFinder();
    alphaModule->setUseVetoCriteria(false);
    add(alphaModule);

    aModule = new GammaFinder();           add(aModule);
    aModule = new CaloClusterFinder();     add(aModule);

    aModule = new Se1e1aCuts();            add(aModule);
