...
TGraphErrors* calculateRatio_OCE(TH1D* hData, int MCHist){
  ...
  for (int bin = 0; bin < hData->GetNbinsX()+1; bin++){
    ...
    if (hData->GetBinContent(bin) > 0){
      ...
      double MCValue = ((TH1D*)StackColl_OCE[MCHist]->GetStack()->Last())->GetBinContent(bin);
	...
	// Switch to next sample at appropriate points (also reset histogram counter)
 	if (i == 0){
	  histStack = ExtStackColl_OCE;
	  denominators = Denominators_Ext;
	  histCnt = 0;
	} else if (i == 21) {
	  histStack = RadStackColl_OCE;
	  denominators = Denominators_Rad;
	  histCnt = 0;
	} else if (i == 38) {
	  histStack = IntStackColl_OCE;
	  denominators = Denominators_Int;
	  histCnt = 0;
	} else if (i == 54){
	  histStack = SigStackColl_OCE;
...
void saveRatioPlots_OCE(TH1D* hData, int MCHist, double xMin, double xMax, double yMin, double yMaxScaler, string name, bool logScale = false){
  ...
  hData->Draw();
  StackColl_OCE[MCHist]->Draw("HISTSAME");
  hData->Draw("SAME");
  if(!logScale){
    Legend_OCE[MCHist]->Draw();
    ChiSquares_OCE[MCHist]->Draw();
  } else {
    LegendLog_OCE[MCHist]->Draw();
    ChiSquaresLog_OCE[MCHist]->Draw();
  }
  c1->Update();
  c1->cd();
  ...
  TGraphErrors* ratio = calculateRatio_OCE(hData, MCHist);
