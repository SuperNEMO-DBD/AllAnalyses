...
TGraphErrors* calculateRatio_1e1g(TH1D* hData, int MCHist){
  ...
  for (int bin = 0; bin < hData->GetNbinsX()+1; bin++){
    ...
    if (hData->GetBinContent(bin) > 0){
      ...
      double MCValue = ((TH1D*)StackColl_1e1g[MCHist]->GetStack()->Last())->GetBinContent(bin);
	...
	// Switch to next sample at appropriate points (also reset histogram counter)
 	if (i == 0){
	  histStack = ExtStackColl_1e1g;
	  denominators = Denominators_Ext;
	  histCnt = 0;
	} else if (i == 21) {
	  histStack = RadStackColl_1e1g;
	  denominators = Denominators_Rad;
	  histCnt = 0;
	} else if (i == 38) {
	  histStack = IntStackColl_1e1g;
	  denominators = Denominators_Int;
	  histCnt = 0;
	} else if (i == 54){
	  histStack = SigStackColl_1e1g;
...
void saveRatioPlots_1e1g(TH1D* hData, int MCHist, double xMin, double xMax, double yMin, double yMaxScaler, string name, bool logScale = false){
  ...
  hData->Draw();
  StackColl_1e1g[MCHist]->Draw("HISTSAME");
  hData->Draw("SAME");
  if(!logScale){
    Legend_1e1g[MCHist]->Draw();
    ChiSquares_1e1g[MCHist]->Draw();
  } else {
    LegendLog_1e1g[MCHist]->Draw();
    ChiSquaresLog_1e1g[MCHist]->Draw();
  }
  c1->Update();
  c1->cd();
  ...
  TGraphErrors* ratio = calculateRatio_1e1g(hData, MCHist);
