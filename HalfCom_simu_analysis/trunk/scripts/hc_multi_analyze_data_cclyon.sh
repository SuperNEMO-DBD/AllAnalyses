#!/usr/bin/env bash
# hc_multi_analyze_data_cclyon.sh
...
function usage(){
echo "--------------"
echo "Goal : Multi Analyze half commissioning simulation data with qsubs at CC Lyon."
...
echo "How to use it"
echo " "
echo "$ ./hc_multi_analyze_data_cclyon.sh [OPTIONS] [ARGUMENTS]"
echo ""
echo "Allowed options: "
echo "-h  [ --help ]     produce help message"
echo "-n  [ --number ]   set the number of events"
echo " "
echo "./hc_multi_analyze_data_cclyon.sh -n number_of_events"
echo "Default value : number_of_events = 10"
echo " "
echo "--------------"
echo "Example : "
echo "./hc_multi_analyze_data_cclyon.sh -n 100000"
echo " "
}
...
SINGLE_SORT_SCRIPT=hc_analyze_data_cclyon.sh
...
do
    ...
    ANALYZED_OUTPUT_PATH="/sps/nemo/scratch/golivier/half_commissioning_simu/damned_simu_${i}/analyzed_data"
    ...
    CONF_DIR=${ANALYZED_OUTPUT_PATH}/conf.d
    LOG_FILE=${LOG_DIR}/multi_analyze.log

    mkdir -p ${ANALYZED_OUTPUT_PATH} ${ANALYZED_ROOT_OUTPUT_PATH} ${ANALYZED_BRIO_OUTPUT_PATH} ${LOG_DIR} ${CONF_DIR}
    if [ $? -eq 1 ];
    then
	echo "ERROR : mkdir ${ANALYZED_OUTPUT_PATH} or ${ANALYZED_ROOT_OUTPUT_PATH} or ${ANALYZED_BRIO_OUTPUT_PATH} or ${LOG_DIR} or ${CONF_DIR} FAILED  !" >> ${LOG_FILE}
	exit 0
    fi    
