...
class Unpack
{
  public:
    ...
    const std::string& getTimeStamp() const { return _dateStamp; }
  ...
  private:
    ...
    std::string _dateStamp;   // annoying ascii date stamp from input.
    double _trigDriftDelay;   // TRIGCONF
    double _alphaRegister;    // TRIGCONF
    int _csmap[20];           // CSMAP readout slot to detector sector.
    ...
    status parseSectorMap(std::string s);
