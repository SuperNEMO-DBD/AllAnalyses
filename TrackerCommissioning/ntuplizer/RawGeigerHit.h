...
class RawGeigerHit
{
  public:
    ...
    //
    void setAddress(int m, int s, int l, int c, int asic);
    void setData(int an, int ct, int cb, int al, int stat);
    ...
    int asicValue() const { return _asicValue; }
    int asicStatus() const { return _asicStatus; }
  ...
  private:
    ...
    int _asicValue;   // raw ASIC value from D0 card.
    int _asicStatus;  // ASIC status word from D1 card. 
    int _anodeTDC;
    int _cathodeTopTDC;
    int _cathodeBotTDC;
    int _alphaTDC;
